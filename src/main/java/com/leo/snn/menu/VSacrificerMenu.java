...
public class VSacrificerMenu extends AbstractContainerMenu {
    ...
    public VSacrificerMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data){
        super(ModMenuTypes.V_SACRIFICER_MENU.get(), pContainerId);

        this.be = ((VSBlockEntity) entity);
    ...
    public VSacrificerMenu(int pContainerId, Inventory inv, FriendlyByteBuf buf) {
        this(pContainerId, inv, inv.player.level().getBlockEntity(buf.readBlockPos()), new SimpleContainerData(11));
    }
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, be.getBlockPos()),
            pPlayer, ModBlocks.V_SACRIFICER.get());
    }

    public VSBlockEntity getBlockEntity() {
        return be;
    }
}
